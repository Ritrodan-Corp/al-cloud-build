...
>
> **Agent bootstrap:** Read **Project State** before substantive work. Agents with private-repo access should use the authoritative internal canon. Before experiment execution, read **Technical Reference → Experiment standards**, including **Experiment execution modes**.
>
> **Operational chronology:** Public build issue #61 is an archived sanitized predecessor. Current operational chronology is private; this public mirror records privacy-safe settled technical state only.
>
...
This section is the privacy-sanitized public current-state surface for AL Cloud. Start here for the objective, active architecture, runtime status, current priorities, operating rules, and ownership model. The private internal canon and private operational chronology are authoritative for current operations; Technical Reference holds durable privacy-safe implementation, evidence, and experiment standards; Experiment History holds completed milestones.
...
Project logging: public build issue #61 is archived. Current operational logging occurs in the private project chronology. Public repository material should contain only settled privacy-safe technical state and provenance that is intentionally public.
...
Priority 1: continue the Pastel optimization program from the retained AOT-N1 renderer SHA 6df5b8af0584d8906eeba2da3bad58d9be30b16e1c227d8d84923808822edfa0 under the retained four-worker SCHED_BATCH measurement policy. The renderer-neutral v8-fast harness governs runtime comparisons. The historical lightweight Reactor cleanup SROA -> SCCP -> SimplifyCFG -> EarlyCSE -> SimplifyCFG -> InstCombine remains the validated compiler win being decomposed. Batch 1 tests E/SE/CE/SCE subsets from selector/postcondition correction commit 65ead9ef36048888e3d1cb6f594978544ef2b559. Corrected build artifacts are complete. Experiment execution may use any reliable authorized mode that satisfies the experiment contract; Scheduled Tasks are optional infrastructure and do not gate approved experiments.
...
Before guest work, verify no other runtime experiment is using the target host. Start paid compute only when guest access is required; keep it stopped during builds, research, document work, and artifact inspection. After user approval, execute an experiment through any reliable authorized mode that can satisfy the experiment contract: a dedicated manual agent, foreground execution when appropriate, a Scheduled Task, or another bounded automation path. Scheduled Tasks have no preferred or mandatory status. Live runtime work remains serialized under one execution owner. Logging is mandatory project recordkeeping but is not an execution gate. Follow the detailed Experiment standards in Technical Reference, preserve the qualified baseline, and restore the starting runtime on exit or failure.
...
Experiment correctness is defined by the experiment contract rather than the orchestration mechanism. After user approval, use any authorized execution mode that can reliably perform the approved scope. Live measurements remain serialized and every mode must verify its starting baseline, preserve source/artifact identity, perform only the approved work, restore the starting runtime on exit or failure, and provide a durable handoff.

Scheduled Tasks remain useful for asynchronous build orchestration, monitoring, progress delivery, and bounded fixed actions. Request-driven build orchestration is proven. Bounded Oracle execution is also proven through an installed fixed-action mailbox bridge: a Scheduled Task performs an ordinary user-space request write, a root-owned allowlisted dispatcher executes only approved fixed actions after integrity checks, and the task reads matching results/logs. This infrastructure remains optional and should not create an experiment dependency when another authorized mode can satisfy the same contract.
...
Public build issue #61 is the archived sanitized predecessor to the current private operational chronology. Historical public issues #59 and #60 and the legacy Drive log remain archival evidence. This public mirror contains settled privacy-safe technical state only; current operational truth belongs in the private internal canon. Detailed completed outcomes belong in Experiment History; current public-safe state belongs in Project State.
...
#### Scheduled Task request-driven orchestration - PASS / OPTIONAL

Request-driven build orchestration was proven with scheduled probe commit 71bccf4b28978cf95d423ca63569ff0b3cc4a8c1 and workflow run 35469164139. Later work also proved bounded fixed-action Oracle execution through the installed allowlisted mailbox bridge. Scheduled Tasks therefore provide a working optional orchestration path for builds, monitoring, progress delivery, and approved bounded actions, without becoming a mandatory experiment-execution model.
