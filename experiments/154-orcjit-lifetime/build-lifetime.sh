...
MESA_BUILD="$ROOT/mesa-build-arm64-orcjit-lifetime"
MESA_INSTALL="$ROOT/mesa-install-arm64-orcjit-lifetime"
CROSS_FILE="$ROOT/aarch64-linux-android.txt"
AUDIT="$ROOT/exp154-audit"
OUT="$ROOT/exp154-mesa-arm64-deploy"
mkdir -p "$AUDIT"
...
replace_once(
    'namespace {\n',
    '''namespace {\n\nstatic std::atomic<unsigned long> alcloud_lifetime_seq{0};\n\nstatic void\nalcloud_lifetime_log(const char *event, const void *lpjit, const void *lljit,\n                     const void *jd, const void *extra)\n{\n   char buffer[512];\n   unsigned long seq = alcloud_lifetime_seq.fetch_add(1, std::memory_order_relaxed) + 1;\n   int len = snprintf(buffer, sizeof(buffer),\n                      "ALCLOUD_ORC_LIFETIME seq=%lu t_ns=%llu pid=%ld tid=%ld event=%s lpjit=%p lljit=%p jd=%p extra=%p\\n",\n                      seq, (unsigned long long)os_time_get_nano(),\n                      (long)getpid(), (long)gettid(), event, lpjit, lljit, jd, extra);\n   if (len > 0) {\n      size_t size = (size_t)len < sizeof(buffer) ? (size_t)len : sizeof(buffer) - 1;\n      (void)write(STDERR_FILENO, buffer, size);\n   }\n}\n''',
    'diagnostic helper')
...
replace_once(
    ...
    '''   static LLVMOrcJITDylibRef create_jit_dylib(const char * name) {\n      using llvm::orc::JITDylib;\n      LPJit* jit = get_instance();\n      alcloud_lifetime_log("CREATE_JD_BEGIN", jit, jit->lljit.get(), NULL, name);\n      JITDylib& tmp = ExitOnErr(jit->lljit->createJITDylib(name));\n      alcloud_lifetime_log("CREATE_JD_DONE", jit, jit->lljit.get(), &tmp, name);\n      return wrap(&tmp);\n   }\n''',
    'create_jit_dylib')
...
replace_once(
    ...
    '''      auto func = ExitOnErr(jit->lljit->lookup(*JD, func_name));\n      jit->lookup_mutex.unlock();\n      alcloud_lifetime_log("LOOKUP_DONE", jit, jit->lljit.get(), JD, func_name);\n''',
    'lookup done')

replace_once(
    '''   static void remove_jd(LLVMOrcJITDylibRef jd) {\n      using llvm::orc::ExecutionSession;\n      using llvm::orc::JITDylib;\n      auto& es = LPJit::get_instance()->lljit->getExecutionSession();\n      ExitOnErr(es.removeJITDylib(* ::unwrap(jd)));\n   }\n''',
    '''   static void remove_jd(LLVMOrcJITDylibRef jd) {\n      using llvm::orc::ExecutionSession;\n      using llvm::orc::JITDylib;\n      LPJit* jit = LPJit::get_instance();\n      alcloud_lifetime_log("REMOVE_JD_BEGIN", jit, jit->lljit.get(), ::unwrap(jd), NULL);\n      auto& es = jit->lljit->getExecutionSession();\n      ExitOnErr(es.removeJITDylib(* ::unwrap(jd)));\n      alcloud_lifetime_log("REMOVE_JD_DONE", jit, jit->lljit.get(), ::unwrap(jd), NULL);\n   }\n''',
    'remove_jd')
...
replace_once(
    ...
    '''   static void init_lpjit() {\n      alcloud_lifetime_log("LPJIT_INIT_BEGIN", NULL, NULL, NULL, NULL);\n      jit = new LPJit;\n      alcloud_lifetime_log("LPJIT_INIT_READY", jit, jit->lljit.get(), NULL, NULL);\n      std::atexit(lpjit_exit);\n      alcloud_lifetime_log("LPJIT_ATEXIT_REGISTERED", jit, jit->lljit.get(), NULL, NULL);\n   }\n''',
    'init_lpjit')

replace_once(
    '''LPJit* LPJit::jit = NULL;\n\nvoid lpjit_exit()\n{\n   delete LPJit::jit;\n}\n''',
    '''LPJit* LPJit::jit = NULL;\n\nLPJit::~LPJit()\n{\n   alcloud_lifetime_log("LPJIT_DTOR_BODY", this, lljit.get(), NULL, NULL);\n}\n\nvoid lpjit_exit()\n{\n   LPJit *jit = LPJit::jit;\n   void *lljit = jit->lljit.get();\n   alcloud_lifetime_log("LPJIT_EXIT_BEGIN", jit, lljit, NULL, NULL);\n   delete jit;\n   alcloud_lifetime_log("LPJIT_EXIT_AFTER_DELETE", jit, lljit, NULL, NULL);\n}\n''',
    'lpjit_exit and destructor')

replace_once(
    '''LPJit::LPJit() :jit_dylib_count(0) {\n   using namespace llvm::orc;\n\n   lp_init_env_options();\n''',
    '''LPJit::LPJit() :jit_dylib_count(0) {\n   using namespace llvm::orc;\n\n   alcloud_lifetime_log("LPJIT_CTOR_BEGIN", this, lljit.get(), NULL, NULL);\n   lp_init_env_options();\n''',
    'LPJit constructor begin')

replace_once(
    '''      .create());\n\n   LLVMOrcIRTransformLayerRef TL = wrap(&lljit->getIRTransformLayer());\n''',
    '''      .create());\n\n   alcloud_lifetime_log("LLJIT_CREATED", this, lljit.get(), NULL, NULL);\n   LLVMOrcIRTransformLayerRef TL = wrap(&lljit->getIRTransformLayer());\n''',
    'LLJIT created')

p.write_text(s)
PY

grep -n 'ALCLOUD_ORC_LIFETIME\|LPJIT_EXIT_BEGIN\|ADD_MAPPING_BEGIN' "$ORC_SRC" \
...
{
  echo 'AL Cloud Experiment 154 ORCJIT lifetime diagnostic bundle'
  echo 'Mesa: 25.0.0'
  echo 'LLVM: 19.1.7'
  echo 'LLVM RTTI: enabled'
  echo 'LLVM Intel JIT events: disabled'
  echo 'JIT backend: ORCJIT / LLJIT'
  echo 'Instrumentation: LPJit/LLJIT lifetime markers only; no behavior fix'
...
BUNDLE="$ROOT/exp154-mesa-25.0.0-android-arm64-llvmpipe-orcjit-lifetime.tar.xz"
tar -C "$ROOT" -cJf "$BUNDLE" "$(basename "$OUT")"
sha256sum "$BUNDLE" | tee "$ROOT/exp154-mesa-bundle.sha256"
ls -lh "$BUNDLE"
