...
ROOT=/home/ritrodano2/exp154
C="$ROOT/candidate/exp154-mesa-arm64-deploy/payload/lib"
AUDIT="$ROOT/candidate/exp154-mesa-arm64-deploy/audit"
...
PROBE_DATA=/var/lib/azl-android/exp154-probe-data-1
GAME_DATA=/var/lib/azl-android/exp154-game-data-1
E="$ROOT/evidence"
...
restore_reference() {
  ...
  sudo -n docker rm -f azl-redroid-exp154-probe azl-redroid-exp154-game >/dev/null 2>&1 || true
...
verify_candidate() {
  ...
  grep -q 'ALCLOUD_ORC_LIFETIME' "$AUDIT/instrumentation-binary-proof.txt"
}
...
if [[ "$MODE" == probe ]]; then
  echo '=== Experiment 154 presentation regression ==='
  ...
  boot_candidate azl-redroid-exp154-probe "$PROBE_DATA"
  ...
  grep 'ALCLOUD_ORC_LIFETIME' "$E/probe-logcat.txt" > "$E/probe-lifetime-markers.txt"
  test -s "$E/probe-lifetime-markers.txt"
  ...
  grep 'ALCLOUD_ORC_LIFETIME' "$E/probe-final-logcat.txt" > "$E/probe-final-lifetime-markers.txt"
...
echo '=== Experiment 154 one-launch game diagnostic ==='
sudo -n systemctl stop azl-redroid.service || true
...
boot_candidate azl-redroid-exp154-game "$GAME_DATA"
...
for i in $(seq 1 120); do
  printf '%03d %s\n' "$i" "$(adb -s "$ADB" shell pidof "$PKG" 2>/dev/null | tr -d '\r' || true)" >> "$E/game-pid-timeline.txt"
  sleep 0.1
done
adb -s "$ADB" exec-out screencap -p > "$E/game-12s.png" || true
adb -s "$ADB" logcat -d -v threadtime > "$E/game-logcat.txt"
adb -s "$ADB" logcat -d -b crash -v threadtime > "$E/game-crash.txt" || true
grep 'ALCLOUD_ORC_LIFETIME' "$E/game-logcat.txt" > "$E/game-lifetime-markers.txt" || true
grep -E 'UnityGfxDeviceW|gallivm_add_global_mapping|ALCLOUD_ORC_LIFETIME|Fatal signal|FORTIFY|SIGSEGV|SIGABRT|libgallium_dri' "$E/game-logcat.txt" > "$E/game-decisive-markers.txt" || true
